...
/**
 * "Install Get Fresh" for a customer who opened the storefront link in a
 * browser rather than the installed app — the same mechanism as the
 * delivery tree's own `InstallPrompt` (src/components/delivery/
 * install-prompt.tsx), kept as its own copy rather than a shared import
 * since the two trees deliberately never share components (M9 — the admin/
 * delivery bundles must never reach a customer phone, and vice versa).
 *
 * Two very different mechanics behind one strip:
 *   - Chrome/Android fires `beforeinstallprompt`, which the shop layout's
 *     pre-hydration script holds onto and this replays when the customer
 *     taps Install — a real one-tap install.
...
// `window`, put there by the shop layout's pre-hydration script, and the
// script announces each capture with `installpromptready`.
    ...
    <div className="flex items-start gap-2 border-b border-border bg-tint-green px-4 py-2.5">
      <div className="min-w-0 flex-1">
        ...
        {/* The one-tap path only exists when Chrome's `beforeinstallprompt`
            ...
            to the menu instructions rather than hiding the strip: a
            customer who was shown nothing at all has no way to install. */}
        {deferred ? (
          <button
            type="button"
            onClick={async () => {
              ...
              // instructions if the customer declined it.
              setUsed(true);
